# Hey, I'm Celestine 👋🏽

I'm a self-taught aspiring AI Engineer and automation developer based in Kenya. 
I'm passionate about building practical software that solves real-world problems, 
with a growing focus on **AI, backend engineering, and automation**.

I learn best by building, breaking things, figuring out why they broke, and building 
them better.

Currently, I'm working on projects that sit at the intersection of **Python, AI, 
and business automation** while working toward becoming a professional AI Engineer.
<p align="center">
  <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExZWZqcmg1Z29wODUwcWZmaHNwd3ZhOWMyNmZpaGE5YmFpc3I4MXd2diZlcD12MV9naWZzX3NlYXJjaCZjdD1n/nQIqq2I47oITIJz52F/giphy.gif" alt="Coding GIF" width="400">
</p>

### 📘 How I'd Define Me... Literally

```python
class Celestine:
    def __init__(self):
        self.name = "Celestine Wangechi"
        self.location = "Nairobi, Kenya"
        self.education = [
            "Self-taught"
        ]

        self.focus = [
            "AI Engineering",
            "Backend Development",
            "Automation",
            "Applied AI"
        ]

        self.currently_building = [
            "TenderLens",
            "Lead-to-Speed Automation",
            "AI-powered business tools"
        ]

    def languages(self):
        return {
            "human": ["English", "Swahili", "Kikuyu"],
            "programming": [
                "Python",
                "SQL",
                "JavaScript"
            ]
        }

    def technologies(self):
        return [
            "Python",
            "FastAPI",
            "Flask",
            "MySQL",
            "PostgreSQL",
            "Git",
            "GitHub"
        ]

    def currently_learning(self):
        return [
            "AI Engineering",
            "Machine Learning",
            "LLM Applications",
            "APIs & Backend Architecture",
            "Data & AI Systems",
            "Automation"
        ]

    def projects(self):
        return {
            "TenderLens":
                "Turning complex tender documents into structured compliance checklists.",

            "Lead-to-Speed":
                "Building automation that helps businesses respond to leads faster.",
        }

    def how_i_build(self):
        return [
            "Start with a real problem.",
            "Understand the system before writing the code.",
            "Build it myself.",
            "Break it when necessary.",
            "Debug it.",
            "Learn from it.",
            "Build something better."
        ]

    def long_term_goal(self):
        return "Become a professional AI Engineer building systems that solve everyday problems."

    def outside_the_code(self):
        return [
            "Learning",
            "Building",
            "Problem solving",
            "Exploring ideas",
            "Trying to turn them into something real"
        ]
```

## Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/celestine-wangechi/)
[![X](https://img.shields.io/badge/X-%23000000.svg?style=for-the-badge&logo=x&logoColor=white)](YOUR_X_URL)
[![Instagram](https://img.shields.io/badge/Instagram-%23E4405F.svg?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/aiwithceleste/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:wangechicelestine@gmail.com)

## Skills & Expertise

<table>
  <tr>
    <td valign="top" width="50%">

**🐍 Languages**
<br/>

[![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)](https://www.python.org/)
[![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)
[![SQL](https://img.shields.io/badge/SQL-%23025E8C.svg?style=for-the-badge&logo=postgresql&logoColor=white)](https://www.postgresql.org/)

<br/>

**⚙️ Backend & Frameworks**
<br/>

[![FastAPI](https://img.shields.io/badge/FastAPI-%23009688.svg?style=for-the-badge&logo=fastapi&logoColor=white)](https://fastapi.tiangolo.com/)
[![Flask](https://img.shields.io/badge/Flask-%23000000.svg?style=for-the-badge&logo=flask&logoColor=white)](https://flask.palletsprojects.com/)
[![MySQL](https://img.shields.io/badge/mysql-%234479A1.svg?style=for-the-badge&logo=mysql&logoColor=white)](https://www.mysql.com/)
[![PostgreSQL](https://img.shields.io/badge/postgresql-%23336791.svg?style=for-the-badge&logo=postgresql&logoColor=white)](https://www.postgresql.org/)

<br/>

**🤖 AI & Machine Learning**
<br/>

[![OpenAI](https://img.shields.io/badge/OpenAI-%23412991.svg?style=for-the-badge&logo=openai&logoColor=white)](https://openai.com/)
[![scikit-learn](https://img.shields.io/badge/scikit--learn-%23F7931E.svg?style=for-the-badge&logo=scikit-learn&logoColor=white)](https://scikit-learn.org/)
[![PyTorch](https://img.shields.io/badge/PyTorch-%23EE4C2C.svg?style=for-the-badge&logo=pytorch&logoColor=white)](https://pytorch.org/)

</td>

<td valign="top" width="50%">

**🔄 Automation & AI Workflows**
<br/>

[![n8n](https://img.shields.io/badge/n8n-%23EA4B71.svg?style=for-the-badge&logo=n8n&logoColor=white)](https://n8n.io/)
[![OpenAI](https://img.shields.io/badge/LLMs-%23412991.svg?style=for-the-badge&logo=openai&logoColor=white)](https://openai.com/)
[![REST API](https://img.shields.io/badge/REST_APIs-%23009688.svg?style=for-the-badge&logo=fastapi&logoColor=white)](#)

<br/>

**🛠️ Developer Tools**
<br/>

[![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)](https://git-scm.com/)
[![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/)
[![VS Code](https://img.shields.io/badge/Visual_Studio_Code-%230078D4.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white)](https://code.visualstudio.com/)

<br/>

**🎯 Areas of Interest**
<br/>

[![AI Engineering](https://img.shields.io/badge/AI_Engineering-7B61FF?style=for-the-badge&logo=robotframework&logoColor=white)](#)
[![Machine Learning](https://img.shields.io/badge/Machine_Learning-%23F7931E.svg?style=for-the-badge&logo=scikit-learn&logoColor=white)](#)
[![NLP](https://img.shields.io/badge/NLP-%234B8BBE.svg?style=for-the-badge&logo=python&logoColor=white)](#)
[![Automation](https://img.shields.io/badge/Automation-%23FF6F00.svg?style=for-the-badge&logo=robotframework&logoColor=white)](#)

</td>
</tr>
</table>
